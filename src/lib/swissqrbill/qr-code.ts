...
import { getReferenceType, mm2pt } from "./utils";
...
export function renderQRCode(
  data: Data,
  size: number,
  renderBlockFunction: (x: number, y: number, blockSize: number) => void
): void {
  const qrData = generateQRData(data);
  const qrCode = QRCode.create(qrData, {
    errorCorrectionLevel: "medium",
    version: 10,
  });

  const blockSize = size / qrCode.modules.size;
